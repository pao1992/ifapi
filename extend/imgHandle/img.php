...
//递归生成文件夹
    ...
    public function water($image) {
        $this->watermark_logo = $this->watermark_logo;
...
//        }
     ...
     * 生成缩略图
     *
     * @param String $imagefile 原始文件,相对路径
    ...
    public function reduceImage($imagefile, $thumbWidth, $thumbHeight, $path = "thumb") {
        ...
        // 生成缩略图
        $dir = date ( "Ym", time () );
        $imagefile =  $imagefile;//被修改成相对路径
        $imagefile_s = "./static/attached/" . $path . "/" . $dir . "/s_" . basename ( $imagefile );//同样使用相对路径
        //生成文件夹
        $this->Directory(dirname($imagefile_s));
        $imagetrans = new img ();
