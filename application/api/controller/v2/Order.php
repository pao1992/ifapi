...
//            ->toArray();
     ...
     * @throws \app\lib\exception\ParameterException
     */
    public function getSummary($page=1, $size = 20,$param = '',$content = ''){
        (new PagingParameter())->goCheck();
//        $uid = Token::getCurrentUid();
        $pagingOrders = OrderModel::getSummaryByPage($page, $size, $param, $content);
        if ($pagingOrders->isEmpty())
        {
            return [
                'current_page' => $pagingOrders->currentPage(),
    ...
    public function updateOne($id){
        ...
        $validate->goCheck();
        $data = $validate->getDataByRule(Request::instance()->param());
